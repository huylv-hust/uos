...
class Controller_KeepList extends Controller_Uos
{
	...
	public function action_index()
	{
		...
		$data['stations'] = Controller_Search::stations($data['jobs']);
		$data['title'] = 'キープリスト';
	...
	public function action_keeplist()
	{
		if(\Fuel\Core\Input::post())
		{
			$job = new Model_Job();
			$return = array();
			if( ! $job_id = \Fuel\Core\Input::post('job_id') or ($job->get_all_job(array('job_id' => $job_id), 'job_id', 'count') == 0))
			{
				return false;
			}
	...
	public function action_check_time()
	{
        ...
        $job = new Model_Job();
        if($job->get_all_job(array('job_id' => $job_id), 'job_id', 'count')) {
