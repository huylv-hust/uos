...
class Model_Contact extends \Fuel\Core\Model_Crud
{
	...
	public function send_mail($data = array())
	{
		...
		\Utility::sendmail($data['email'],'【おしごとnavi】お問い合わせありがとうございます',$data,'mail/contact-thanks');
		...
		if($emails = $user->get_list_user_email() and ! empty($emails))
		{
			...
			\Utility::sendmail($listmail,'【おしごとnavi】お問い合わせがありました',$data,'mail/contact-notice');
		}
